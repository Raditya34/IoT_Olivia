...
class MqttSubscribe extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $this->mqtt->subscribe($subscribeTopik, function (string $topic, string $message) use ($publishTopik) {
                // ✅ FIX: bungkus SELURUH isi handler dengan try/catch Throwable.
                // Kalau ada error apapun di dalam sini (termasuk fatal \Error seperti
                // "Call to undefined method"), proses subscriber TIDAK akan mati total.
                // Cuma 1 pesan ini yang gagal diproses, listener tetap lanjut jalan.
                try {
                    $parts      = explode('/', $topic);
                    ...
                    // ✅ FIX UTAMA: sebelumnya jalur ini gak dijaga sama sekali. Kalau
                    // simpanKeDatabase() atau method lain manapun di dalam handler ini
                    // melempar \Error (bukan \Exception, misal "Call to undefined method"),
                    // itu akan lolos dari catch-catch kecil di atas dan bikin SELURUH
                    // proses mqtt:subscribe mati (fatal), tanpa log yang jelas soal DB.
                    // Dengan \Throwable di sini, error apapun ketangkep dan di-log,
                    // listener tetap hidup untuk pesan berikutnya.
                    $this->error("FATAL di dalam handler MQTT [$topic]: " . $e->getMessage());
                    \Log::error("MQTT handler crash pada topic [$topic]: " . $e->getMessage(), [
            ...
            // ✅ FIX: \Exception -> \Throwable, supaya error koneksi level rendah
            // (termasuk \Error) tetap ke-log dengan jelas alih-alih silent crash.
            $this->error("Koneksi Error: " . $e->getMessage());
            \Log::error("MQTT koneksi/loop crash: " . $e->getMessage(), [
                'trace' => $e->getTraceAsString(),
            ]);
        }
    }
     ...
     * ✅ FIX: sebelumnya method ini di-comment habis sehingga pemanggilannya
     * di atas melempar fatal "Call to undefined method" setiap kali ada device
     * yang kirim payload FLAT (bukan nested). Sekarang diaktifkan lagi, dan
     * memakai retryCreate() (bukan ::create() polos) supaya jalur modular ini
     * punya proteksi retry Railway timeout yang sama seperti jalur nested (OPSI A).
     */
    private function simpanKeDatabase(string $deviceCode, array $data): void
    {
        match ($deviceCode) {
            'OLIVIA-01', 'OLIVIA-MASTER' => $this->retryCreate('Arang', fn() => Esp1Arang::create([
                'suhu_arang'   => $data['suhu_arang']   ?? 0,
                'volume_arang' => $data['volume_arang'] ?? 0,
            ])),
            'OLIVIA-02' => $this->retryCreate('Bleaching', fn() => Esp2Bleaching::create([
            ...
            'OLIVIA-03' => $this->retryCreate('Validasi', fn() => Esp3Validasi::create([
    ...
    private function retryCreate(string $label, \Closure $callback, int $maxAttempts = 3, int $delayMs = 300)
    {
        ...
        while (true) {
            ...
            try {
                ...
                // ✅ FIX: \Exception -> \Throwable di sini juga, supaya semua jenis
                // error DB (termasuk \Error) tetap kena log yang jelas per-label,
                // bukan cuma yang turunan \Exception.
                ...
                if ($attempt >= $maxAttempts || !$isConnIssue) {
                    ...
                    \Log::error("Gagal DB $label: " . $e->getMessage());
                    return null;
                }
